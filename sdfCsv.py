...
class SdfCsv():
    ...
    def __init__(self
        ...
        .. note:: SIR 3S imports SDF-CSV-Files. However sometimes it is convenient to have a look at the original SDF-CSV-File content with Python. I.e. to explore the original hierarchical layer structure in comparison to the flat layer structure in the SIR 3S model. Or i.e. to test the SIR 3S SDF-CSV-File export. In general, one can say that the SDF-CSV format is so widespread in Germany that it makes sense to have an SIR 3S affine import function to pandas.
