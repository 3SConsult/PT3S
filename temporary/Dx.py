        ...
        try:
            ...
            for edge in [edge for edge in dfVBEL.index.unique(level=0).to_list()]:
                  try:    
                    ...
                    df=dfOBJTYPEs[edge]
                    logger.debug("{0:s}{1:s}: \n{2}".format(logStr, 'DataFrame content prior to calling construct multiindex', df))
                    ...
                    #df = Xm.Xm.constructNewMultiindexFromCols(df=df, mColNames=['OBJTYPE', 'pk'], mIdxNames=['OBJTYPE', 'OBJID'])
                    
                    #df=pd.merge(dfVBEL.loc[(edge,),:],df,left_index=True,right_index=True)
                    #dfs.append(df)
                    if not df.isnull().values.any():
                        # Append df to a list or perform further processing
                        dfs.append(df)
                        logger.debug("DataFrame appended successfully.")
                    else:
                        logger.debug("DataFrame contains NaT values, not appending.") 
            ...
            if len(dfs) > 0:
                # Log the DataFrames in dfs
                logger.debug("DataFrames in dfs:")
                for i, valid_df in enumerate(dfs):
                    logger.debug(f"DataFrame {i}: \n{valid_df}")
                # Log dfVBEL before merging
                logger.debug("dfVBEL before merging: \n{0}".format(dfVBEL))
                dfs_concat=pd.concat(dfs);
                # Concatenate the DataFrames
                if dfs:
                    dfs_concat = pd.concat(dfs)
                    logger.debug("Concatenated DataFrame (dfs_concat): \n{0}".format(dfs_concat))
                else:
                    dfs_concat = pd.DataFrame()
                    logger.debug("No valid DataFrames to concatenate.")
                try:
                    dfVBEL = dfVBEL.join(dfs_concat, how='left')
                    logger.debug("dfVBEL after merging: \n{0}".format(dfVBEL))
                except Exception as e:
