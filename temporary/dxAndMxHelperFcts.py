...
class dxWithMx():
    ...
    def _V3_AGSNVEC(self,V3_AGSN):
        ...
        try: 
            ...
            # Check if self.dx.QGISmodelXk is None
            if self.dx.QGISmodelXk is None:
                flowMVEC = 'QMVEC'
                logger.debug(f"{logStr}self.dx.QGISmodelXk is None. Using default (m3/h). {flowMVEC} wird verwendet fuer das Mapping in VBEL-QM.")
            else:
                try:
                    vm = self.dx.dataFrames['VIEW_MODELLE']
                    vms = vm[vm['pk'].isin([self.dx.QGISmodelXk])].iloc[0]
                    ...
                    logger.debug(f"{logStr}IDQM konnte nicht ermittelt werden. {flowMVEC} wird verwendet fuer das Mapping in VBEL-QM. Exception: {e}")
            ...
            # VEC source cols "flow": calced by PT3S
            for col, colVECType in zip(['QM'], [self.flowMVEC]):
                logger.debug(f"Checking if column '{col}' exists in dfAGSNVec.")
                if col not in dfAGSNVec.columns:
                    logger.debug(f"Column '{col}' does not exist in dfAGSNVec. Available columns: {dfAGSNVec.columns}")
                    continue
                self._V3_AGSNVEC_mapExistingColWithVecResults(dfAGSNVec, col, 'STAT', colVECType, t0rVec, t0rVec)
...
def constructNewMultiindexFromCols(df=pd.DataFrame(),mColNames=['OBJTYPE','OBJID'],mIdxNames=['OBJTYPE','OBJID']):
        ...
        try:
            logger.debug("{0:s}{1:s}: \n{2}".format(logStr, 'DataFrame content before constructmultiindex', df))
            
            arrays = []
            for col in mColNames:
                logger.debug(f"{logStr}{col}: {type(df[col])}")
                logger.debug(f"{logStr}{col}: {type(df[col])}")
                logger.debug("{0:s}{1:s}: \n{2}".format(logStr, 'DataFrameCOl content to be appended to arrays', df[col]))
                
                # Use a local variable for the column
                col_data = df[col]
                
                # Check if the column is a DataFrame and convert it to a Series if necessary
                if isinstance(col_data, pd.DataFrame):
                    logger.debug(f"{logStr}{col} is a DataFrame, converting to Series.")
                    col_data = col_data.squeeze()  # Explicitly convert to Series
                
                logger.debug(f"{logStr}{col}: {type(col_data)}")
                logger.debug("{0:s}{1:s}: \n{2}".format(logStr, 'DataFrame[Col] content to be appended to arrays(after datatype change)', col_data))
                arrays.append(col_data.tolist())
            
            logger.debug(f"{logStr}arrays: {arrays}")
            
            # Detailed logging for the next steps
            tuples = list(zip(*(arrays)))
            logger.debug(f"{logStr}tuples: {tuples}")
            
            index = pd.MultiIndex.from_tuples(tuples, names=mIdxNames)
            logger.debug(f"{logStr}MultiIndex: {index}")
            
            df.drop(mColNames, axis=1, inplace=True)
            logger.debug(f"{logStr}DataFrame after dropping columns: \n{df}")
            
            df = pd.DataFrame(df.values, index=index, columns=df.columns)
            logger.debug("{0:s}{1:s}: \n{2}".format(logStr, 'DataFrame content after construct multiindex', df))
